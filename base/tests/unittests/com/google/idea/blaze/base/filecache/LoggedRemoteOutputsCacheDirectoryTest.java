...
import com.google.idea.blaze.base.filecache.RemoteOutputsCache.LoggedRemoteOutputsCacheDirectory;
...
/** Tests for {@link LoggedRemoteOutputsCacheDirectory}. */
@RunWith(JUnit4.class)
public class LoggedRemoteOutputsCacheDirectoryTest {
  ...
  private final LoggedDirectoryProvider directoryProvider = new LoggedRemoteOutputsCacheDirectory();
  ...
  @Test
  public void directoryIsNotProvidedWhenSettingsNotAvailable() {
  ...
  public void loggedDirectoryHasCorrectPurpose() {
    ...
    assertThat(loggedDirectory).isPresent();
    assertThat(loggedDirectory.get().purpose()).isEqualTo("Remote outputs cache");
  }
  ...
  private BlazeImportSettings createSettings(BuildSystem buildSystem) {
    return new BlazeImportSettings(
        "", "", /* projectDataDirectory= */ temporaryFolder.getRoot().toString(), "", buildSystem);
  }
}
